...
class Strain():
    ...
    def __init__(self, code=None, N_pop=100, fitness=0, mu=2e-5, t_0=0,
        ...
        if table is None:
            code = 'Standard Code'
            table = CodonTable(utils.standard_table)
        ...
        self.t_est = t_est
        self.table = table
        self.mu = self.get_mut_rate(mu)
        self.ID = str(ID)
        self.lineage = lineage
        self.timepoints = []
        self.poptrace = []

    def get_mut_rate(self, mu):
        ...
        table = self.table.codon_dict
        sc = utils.standard_table
        ...
        # if not, calculate how many substitutions are available to each code
            ...
            for (c1, c2) in utils.triplet_mut_pairs:
            ...
            f_sc = count_sc / len(utils.triplet_mut_pairs)
